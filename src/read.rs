...
pub fn read_metadata(conn: &rusqlite::Connection) -> Result<Metadata, Box<dyn std::error::Error>> {
    ...
    while let Some(row) = rows.next()? {
        ...
        match name.as_str() {
            ...
            "bounds" => {
                ...
                if split.len() == 4 {
                    ...
                    if let (Ok(left), Ok(bottom), Ok(right), Ok(top)) = bounds {
                        let tl_br = (
                            GeoCoord::from_degrees(left, top),
                            GeoCoord::from_degrees(right, bottom),
                        );
            ...
            unknown_key => { metadata.custom.insert(unknown_key.to_owned(), value); }
        }
    }
...
pub fn read_tile(conn: &rusqlite::Connection, tile_id: TmsTileId) -> rusqlite::Result<Option<Vec<u8>>> {
    let mut select_tile = conn.prepare_cached("SELECT tile_data FROM tiles WHERE zoom_level = ?1 AND tile_column = ?2 AND tile_row = ?3")?;
...
pub fn read_grid(conn: &rusqlite::Connection, tile_id: TmsTileId) -> rusqlite::Result<Option<Vec<u8>>> {
    let mut select_grid = conn.prepare_cached("SELECT grid FROM grids WHERE zoom_level = ?1 AND tile_column = ?2 AND tile_row = ?3")?;
...
pub fn read_grid_data(conn: &rusqlite::Connection, tile_id: TmsTileId, key: &str) -> rusqlite::Result<Option<String>> {
    let mut select_grid = conn.prepare_cached("SELECT key_json FROM grid_data WHERE zoom_level = ?1 AND tile_column = ?2 AND tile_row = ?3 AND key = ?4")?;
